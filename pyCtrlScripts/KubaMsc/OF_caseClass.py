...
class OpenFOAMCase:
    def __init__(self):
        ...
        print(
            "New OpenFOAMCase has been initialized, starting directory is:\n\t%s"
            % self.whereIStart
        )
    ...
    def changeOFCaseDir(self, dir):
        ...
        self.dir = dir
        print(
            "OpenFOAMCase directory has been changed from:\n\t%s to %s"
            % (self.baseDir, dir)
        )
    ...
    def copyBaseCase(self):
        ...
        print(
            "OpenFOAMCase.baseDir has been copied to OpenFOAMCase.dir:\n\t %s --> %s"
            ...
            for lnI in range(len(linesInFl)):
                for whatI in range(len(what)):
                    ...
                    if whatTu in linesInFl[lnI]:
                        ...
                        print(
                            "In %s, I have replaced %s by %s on line %d."
                            % (inFl, whatTu, byTu, lnI)
                        )
    ...
    def setParameters(self, inParVals):
        ...
        for inParVal in inParVals:
            ...
            print(
                "In %s, trying to change parameter %s in subDict %s:"
                % (inFl, par, inSubDict)
            )
            with open(inFl, "r") as fl:
                content = fl.read()
                contentN = re.sub(
                    r"(%s)(\s+%s\s+[^\]]*]\s+|\s+)([\S ]+;)" % (par, par),
                    r"\1 \2 %s;" % (val),
                    content,
                )
                matches = re.finditer(
                    r"(%s)(\s+%s\s+[^\]]*]\s+|\s+)([\S ]+;)" % (par, par), content
                )
            if not inSubDict == "":
                match1 = re.search(r"%s\s*{([^}^{]*)[}{]" % inSubDict, content)
                contentMatchN = re.sub(
                    r"(%s)(\s+%s\s+[^\]]*]\s+|\s+)([\S ]+;)" % (par, par),
                    r"\1 \2 %s;" % (val),
                    match1.group(0),
                )
                ...
                contentN = re.sub(
                    r"%s\s*{([^}^{]*)[}{]" % inSubDict, contentMatchN, content
                )
            for match in matches:
                print(
                    "\tIn %s, I have change val of %s parameter from %s to %s in subDict %s."
                    % (inFl, par, match.group(3), val, inSubDict)
                )
            with open(inFl, "w") as fl:
                fl.write(contentN)
            ...
            if not inSubDict == "":
                ...
                for lnI in range(len(linesInFl)):
                    if lnI < subDictEnd:
                        ...
                        print(
                            "In %s, I have added line %s to subdictionary %s on line %d"
                            % (inFl, strToAdd, inSubDict, lnI)
                        )
                        newLinesInFl.append(linesInFl[lnI])
                    else:
                        newLinesInFl.append(linesInFl[lnI])
    ...
    def updateTimes(self):
        ...
        print(
            "Found OpenFOAMCase times are:\n\t%s,\nlatestTime:\n\t%g"
            % (str(self.times), self.latestTime)
        )

        # -- move back where I start
        os.chdir(self.whereIStart)

    def updateTimesParallel(self):
        """update the OpenFOAMCase.times variable"""

        # -- move to OpenFOAMCase directory
        os.chdir(self.dir)

        lstDir = os.listdir("processor0")
        flLstDir = [float(flDir) for flDir in lstDir if isFloat(flDir)]
        self.parTimes = flLstDir
        self.latestParTime = max(flLstDir)
        print(
            "Found OpenFOAMCase parallel times are:\n\t%s,\nlatestTime:\n\t%g"
            % (str(self.parTimes), self.latestParTime)
        )
