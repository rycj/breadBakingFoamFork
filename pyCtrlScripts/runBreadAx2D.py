...
if prepBlockMesh:
    prep2DMeshZhang(
        arcL, rLoaf, hLoaf, x0, y0, z0, dA, dX, dY, dZ, grX, grY, grZ, baseCase
    )
...
if runPostProcess:
    ...
    TExpCenter = np.loadtxt(
        baseCaseDir + "ZZ_dataForPostProcessing/exp_Zhang_center.dat", delimiter=";"
    )
    TExpSurface = np.loadtxt(
        baseCaseDir + "ZZ_dataForPostProcessing/exp_Zhang_surface.dat", delimiter=";"
    )
    DExp = np.loadtxt(
        baseCaseDir + "ZZ_dataForPostProcessing/exp_Zhang_D.dat", skiprows=1
    )
    moistureExp = np.loadtxt(
        baseCaseDir + "ZZ_dataForPostProcessing/exp_Zhang_moisture.dat",
        skiprows=1,
        delimiter=";",
    )
    ...
    probesT = np.loadtxt(
        baseCase.dir + "/postProcessing/probeZhang/%d/T" % latestTime, skiprows=3
    )
    ...
    # -- Temperatures
    axs[0].plot(
        TExpCenter[:, 0], TExpCenter[:, 1], "xr", label="center temperature experiment"
    )
    axs[0].plot(
        TExpSurface[:, 0],
        TExpSurface[:, 1],
        "xb",
        label="surface temperature experiment",
    )
    axs[0].plot(
        probesT[:, 0] / 60,
        probesT[:, 1] - 273,
        "r",
        label="center temperature simulation",
    )
    axs[0].plot(
        probesT[:, 0] / 60,
        probesT[:, 2] - 273,
    ...
    axs[2].set_xlim(0, 15)
    axs[2].set_title(
        "Displecement of the bread in vertical (X) and horizontal (Y) directions"
    )
    axs[2].legend()
    fig.tight_layout()
